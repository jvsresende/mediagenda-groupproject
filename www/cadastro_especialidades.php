...
function voltarEspecialidades($tipo, $msg) {
    header("Location: cadastro_especialidades.php?tipo=" . urlencode($tipo) . "&msg=" . urlencode($msg));
    exit;
}
...
$operadorNome = "";
$operadorEmail = "";
$usuarioResult = mysqli_query($conexao_bd, "SELECT nome, email FROM usuario WHERE cod_usuario = $cod_usuario");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'novo' || $acao === 'editar') {
        ...
        if ($nome === '') {
            voltarEspecialidades('erro', 'Preencha o nome da especialidade.');
        }

        if ($acao === 'novo') {
            $ok = mysqli_query($conexao_bd, "INSERT INTO especialidades (nome, status) VALUES ('$nome', '$status')");
            voltarEspecialidades($ok ? 'sucesso' : 'erro', $ok ? 'Especialidade cadastrada com sucesso.' : 'Nao foi possivel cadastrar. Verifique se o nome ja existe.');
        }

        if ($id <= 0) {
            voltarEspecialidades('erro', 'Especialidade invalida.');
        }

        $ok = mysqli_query($conexao_bd, "UPDATE especialidades SET nome = '$nome', status = '$status' WHERE id = $id");
        voltarEspecialidades($ok ? 'sucesso' : 'erro', $ok ? 'Especialidade atualizada com sucesso.' : 'Nao foi possivel atualizar a especialidade.');
    }
    ...
    if ($acao === 'inativar') {
        ...
        $ok = mysqli_query($conexao_bd, "UPDATE especialidades SET status = 'Inativo' WHERE id = $id");
        voltarEspecialidades($ok ? 'sucesso' : 'erro', $ok ? 'Especialidade inativada com sucesso.' : 'Nao foi possivel inativar a especialidade.');
    }
    ...
    if ($acao === 'excluir') {
        ...
        $medicos = mysqli_fetch_assoc(mysqli_query($conexao_bd, "SELECT COUNT(*) AS total FROM medicos WHERE especialidade_id = $id"));
        $agendamentos = mysqli_fetch_assoc(mysqli_query($conexao_bd, "SELECT COUNT(*) AS total FROM agendamentos WHERE especialidade_id = $id"));
        if ((int)$medicos['total'] > 0 || (int)$agendamentos['total'] > 0) {
            voltarEspecialidades('erro', 'Esta especialidade possui vinculos. Use inativar para manter o historico.');
        }
        $ok = mysqli_query($conexao_bd, "DELETE FROM especialidades WHERE id = $id");
        voltarEspecialidades($ok ? 'sucesso' : 'erro', $ok ? 'Especialidade excluida com sucesso.' : 'Nao foi possivel excluir a especialidade.');
    }
}
...
if ($filtroStatus !== '') {
    ...
    $where[] = "e.status = '$statusBusca'";
}
$whereSql = count($where) ? "WHERE " . implode(" AND ", $where) : "";

$especialidades = array();
$sql = "SELECT e.id, e.nome, e.status,
...
<head>
    ...
    <style>
        ...
        .tabela-especialidades { width:100%; border-collapse:separate; border-spacing:0; font-size:0.88rem; }
        .tabela-especialidades thead th { background:var(--azul-claro); color:var(--azul-escuro); font-weight:600; padding:10px 14px; border-bottom:2px solid var(--cinza-borda); white-space:nowrap; }
        .tabela-especialidades tbody tr:hover { background:#f8fbff; }
        .tabela-especialidades tbody td { padding:10px 14px; border-bottom:1px solid var(--cinza-borda); vertical-align:middle; }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        <ul class="nav flex-column">
            ...
            <li class="nav-item"><a class="nav-link" href="cadastro_medicos.php"><i class="fa-solid fa-user-doctor"></i> Cadastro de Medicos</a></li>
    ...
    <main class="conteudo-principal" id="conteudoPrincipal">
            ...
            <form method="GET" action="cadastro_especialidades.php">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-sm" id="filtroNome" name="nome" placeholder="Nome da especialidade" value="<?php echo h($filtroNome) ?>">
                    </div>
                    <div class="col-md-3">
        ...
        <div class="card-pagina">
            <div class="card-titulo d-flex justify-content-between align-items-center">
                ...
                <span class="text-muted" style="font-size:0.82rem; font-weight:400;"><?php echo count($especialidades) ?> registro(s) encontrado(s)</span>
            </div>
            <div class="table-responsive">
                <table class="tabela-especialidades">
                    <thead>
                        <tr>
                            <th>#</th><th>Nome</th><th>Medicos</th><th>Agendamentos</th><th>Status</th><th class="text-center">Acoes</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (empty($especialidades)): ?>
                            <tr><td colspan="6" class="text-center text-muted py-4"><i class="fa-solid fa-list-check me-2"></i>Nenhuma especialidade encontrada.</td></tr>
                        <?php else: ?>
                            <?php foreach ($especialidades as $esp):
                                $classeBadge = $esp['status'] === 'Ativo' ? 'badge-ativo' : 'badge-inativo';
                            ?>
                                <tr>
                                    <td class="text-muted"><?php echo (int)$esp['id'] ?></td>
                                    <td><?php echo h($esp['nome']) ?></td>
                                    <td><?php echo (int)$esp['total_medicos'] ?></td>
                                    <td><?php echo (int)$esp['total_agendamentos'] ?></td>
                                    <td><span class="badge-status <?php echo $classeBadge ?>"><?php echo h($esp['status']) ?></span></td>
                                    <td class="text-center" style="white-space:nowrap;">
                                        <button class="btn btn-sm btn-outline-primary py-0 px-2 btn-editar" title="Editar" data-id="<?php echo (int)$esp['id'] ?>" data-nome="<?php echo h($esp['nome']) ?>" data-status="<?php echo h($esp['status']) ?>"><i class="fa-solid fa-pen"></i></button>
                                        <button class="btn btn-sm btn-outline-warning py-0 px-2 btn-inativar" title="Inativar" data-id="<?php echo (int)$esp['id'] ?>" data-nome="<?php echo h($esp['nome']) ?>"><i class="fa-solid fa-ban"></i></button>
                                        <button class="btn btn-sm btn-outline-danger py-0 px-2 btn-excluir" title="Excluir definitivamente" data-id="<?php echo (int)$esp['id'] ?>" data-nome="<?php echo h($esp['nome']) ?>"><i class="fa-solid fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalFormTitulo"><i class="fa-solid fa-plus me-2"></i>Nova Especialidade</h5>
    ...
    <form id="formAcaoLinha" action="cadastro_especialidades.php" method="POST" class="d-none">
    ...
    <script>
        ...
        document.querySelector('.tabela-especialidades').addEventListener('click', function(e) {
            ...
            if (btnInativar || btnExcluir) {
                ...
                Swal.fire({
                    title: acao === 'inativar' ? 'Inativar especialidade?' : 'Excluir especialidade?',
